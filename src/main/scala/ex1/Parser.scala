...
trait NotTwoConsecutive[T] extends Parser[T]:
  ...
  private[this] var lastElement: Option[T] = None
  abstract override def parse(t: T): Boolean =
    if (lastElement.isDefined && lastElement.get == t) twoConsecutive = true
    lastElement = Some(t)
    super.parse(t)
...
@main def checkParsers(): Unit =
  ...
  //NotTwoConsecutiveParser -> NotTwoConsecutive[Char] -> BasicParser -> Parser[Char]
  def parserNTC = new NotTwoConsecutiveParser(Set('X', 'Y', 'Z'))
  println("NonConsecutive: ")
  ...
  //new BasicParser(Set('X', 'Y', 'Z')) -> NotTwoConsecutive[Char] -> NonEmpty[Char] -> BasicParser -> Parser[Char]
  def parserNTCNE = new BasicParser(Set('X', 'Y', 'Z'))
    with NotTwoConsecutive[Char]
    with NonEmpty[Char]
  println("NonEmptyNonConsecutive: ")
